...
func init() {
	...
	keystorePath := "./shared_keystore"
	sharedEthash = New(sharedConfig, nil, false, keystorePath)
}
...
type Ethash struct {
	...
	minerAddresses           map[common.Hash]common.Address
	minerLock                sync.RWMutex
	lastExternalMinerAddress common.Address
	logOnce                  sync.Map
...
// packages.
func New(config Config, notify []string, noverify bool, keystorePath string) *Ethash {
	if config.Log == nil {
		config.Log = log.Root()
	}
	...
	if ks == nil {
		config.Log.Crit("Failed to initialize keystore", "path", keystorePath)
	}
	...
	ethash := &Ethash{
		...
		minerAddresses:           make(map[common.Hash]common.Address),
		minerLock:                sync.RWMutex{},
		lastExternalMinerAddress: common.Address{},
		keystore:                 ks,
	}
...
func NewTester(notify []string, noverify bool) *Ethash {
	...
	return New(Config{PowMode: ModeTest}, notify, noverify, keystorePath)
}
